  ...
  return (
    ...
    >
      <a href={href} target="_blank" rel="noopener noreferrer">
        <PlayStoreIcon className={isLarge ? "size-7" : "size-5"} />
        <div className="text-left flex flex-col items-start justify-center pr-2">
          <span className={isLarge ? "text-xs leading-none font-light tracking-tighter" : "text-[10px] leading-none font-light tracking-tighter"}>
            GET IT ON
          </span>
          <p className={isLarge ? "text-lg font-bold leading-none" : "text-base font-bold leading-none"}>Google Play</p>
        </div>
      </a>
    </Button>
  );
